...
TIME_CLASS = os.getenv("PYTORY_TIME_CLASS", "calendar__time")
CURRENCY_CLASS = os.getenv("PYTORY_CURRENCY_CLASS", "calendar__currency")
PREV_CLASS = os.getenv("PYTORY_PREVIOUS_CLASS_CLASS", "calendar__previous")
ACTUAL_CLASS = os.getenv("PYTORY_ACTUAL_CLASS", "calendar__actual")
FORECAST_CLASS = os.getenv("PYTORY_FORECAST_CLASS", "calendar__forecast")
IMPACT_CLASS = os.getenv("PYTORY_IMPACT_CLASS", "calendar__impact")
EVENT_CLASS = os.getenv("PYTORY_EVENT_CLASS", "calendar__event")
ROW_ID_ATTR = os.getenv("PYTORY_ROW_ID_ATTR", "data-event-id")
    ...
    if year is None:
        ...
        if yr_match:
            try:
                ...
                logger.error("Could not extract year from url: %s", e)
                year = None
...
def parse_calendar_from_html(html, url):
    ...
    for row in rows:
        try:
            ...
            logger.exception("Failed to parse one event row, skipping. Error: %s", e)
            continue
    ...
    try:
        ...
        raise ValueError("Failed to convert records to Models") from e
